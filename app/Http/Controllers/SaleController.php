...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                $nestedData['staff_note'] = $sale->staff_note;
    ...
    public function store(Request $request)
    {
        ...
        $rates['service_charge'] = $general_setting->service_charge;
        $rates['vat_rate'] = $general_setting->vat;
        
        $data['rates'] = json_encode($rates);

        // our sub_total_price is the 'total_price' without VAT
        $data['sub_total_price'] = round($data['sub_total_price']); 
        $data['total_price'] = round($data['total_price']);  //total_price with VAT only
        
        $data['grand_total'] = $data['grand_total'];  //grand_total price is the total_price with VAT + discount
        ...
        // edit: 26 feb 2022
        $mail_data['sub_total_price'] = $lims_sale_data->sub_total_price;
        $mail_data['vat'] = $lims_sale_data->vat;
        ...
        /*additional shop_sales table code added by salman starts*/
        $product_id_shop = $request->product_id;
        $product_qty_shop = $request->qty;
        $product_paid_amount = $request->subtotal;
        $sale_table_id = Sale::where('reference_no', $data['reference_no'])
            ->select('id')
            ->first();

        $warehouse_id = [];
        for ($i = 0; $i < count($product_id_shop); $i++) {
            $warehouse_id[$i] = Product_Warehouse::where('product_id', $product_id_shop[$i])
                ->select('warehouse_id')
                ->first();
        }
        // return $warehouse_id;
        ...
        for ($i = 0; $i < count($product_id_shop); $i++) {
            DB::table('shop_sales')->insert([
                'sales_id' => $sale_table_id['id'],
                'product_id' => $product_id_shop[$i],
                'wearhouse_id' => $warehouse_id[$i]['warehouse_id'],
                'reference_no' => $data['reference_no'],
                'total_quantity' => $product_qty_shop[$i],
                'paid_amount' => $product_paid_amount[$i],
            ]);
        }
        /*additional shop_sales table code added by salman end*/
        ...
        foreach ($product_id as $i => $id) {
            ...
            if ($sale_unit[$i] != 'n/a') {
                ...
                if ($data['sale_status'] == 1) {
                    ...
                    for ($i = 0; $i < count($product_id_shop); $i++) {
                        ...
                        $product_qty=Product_Warehouse::where('product_id',$product_id_shop[$i])
                            ->select('qty')
                            ->first();
                        $updated_qty=$product_qty->qty - $quantity;
                        Product_Warehouse::where('product_id',$product_id_shop[$i])
                            ->update([
                                'qty'=>$updated_qty
                            ]);
                    }*/
            ...
            $product_sale['warehouse_id'] = $warehouse_id[$i]['warehouse_id'];
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        // Product Price edit: 26 feb 2022 

        if ($lims_product_data->promotion && $todayDate <= $lims_product_data->last_date) {
            $product[] = $lims_product_data->promotion_price;
        } else {
            $product[] = $lims_product_data->price; //previous code
        }

        //new code
        // $general_setting_data = GeneralSetting::latest()->first();
        // $percentage = $general_setting_data->service_charge / 100; // 6 percentage means 6/100 = 0.6

        // if ($lims_product_data->promotion && $todayDate <= $lims_product_data->last_date) {
        //     $lims_product_data->promotion_price = $lims_product_data->promotion_price + ($lims_product_data->promotion_price * $percentage);
        //     $product[] = $lims_product_data->promotion_price;
        // } else {
        //     $lims_product_data->price = $lims_product_data->price + ($lims_product_data->price * $percentage);
    ...
    public function productSaleData($id)
    {
        ...
        foreach ($lims_product_sale_data as $key => $product_sale_data) {
            ...
            $product_sale[7][$key] = Str::before(Warehouse::find($product_sale_data->warehouse_id)->name, ' ');
        }
        return $product_sale;
    }
    ...
    public function genInvoice($id)
    {
        ...
        $shopExplode = explode(' ', $lims_warehouse_data->name);
        $shop_name = $shopExplode[0];

        $card_number = '**** **** **** ' . substr($lims_sale_data->staff_note, -4);
        return view('sale.invoice', compact('general_setting','shop_name', 'card_number', 'lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
    ...
    public function quntity_update()
    {
        $shops_sales_data = DB::table('shop_sales')->get();
        $i = 0;
        $j = 0;

        foreach ($shops_sales_data as $value) {

            $shop_sales_id = $value->id;
            $product_id = $value->product_id;
            $total_quantity = $value->total_quantity;
            $paid_amount = $value->paid_amount;


            $base_price = DB::table('products')
                ->where('id', $product_id)
                ->select('price')
                ->get();

            if ($base_price->count() == 0) {
                $data = [
                    'error_type' => 'base price not found',
            ...
            $qty = (int) $paid_amount / $base_price[0]->price;

            if (!is_int($qty)) {
                $info = [
                    'error_type' => 'qty not integer',
                    'shop_sales_id' => $shop_sales_id,
                    'product_id' => $product_id,
                    'total_quantity' => $total_quantity,
                    'paid_amount' => $paid_amount,
                    'product_base_price' => $base_price[0]->price,
                    'quantity' => $qty,
                ];
                Log::debug($info);
                $j++;
        ...
        echo ('base price not found = ' . $i);
        echo ('<br>');
        echo ('message between value');
        echo ('<br>');
        echo ('qty not integer = ' . $j);
        echo ('<br>');
