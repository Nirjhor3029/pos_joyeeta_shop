...
class ReportController extends Controller
{
    ...
    public function prep()
    {
        ...
        ->select(
            ...
            ->join("products", "products.id", "=", "shop_sales.product_id")
            ->join("warehouses", "warehouses.id", "=", "shop_sales.wearhouse_id")
            ->join("sales", "sales.id", "=", "shop_sales.sales_id")
            ->get();
    }
    ...
    public function purchaseReport(Request $request)
    {
        ...
        $select = [
            ...
            "shop_sales.total_quantity",
            "shop_sales.paid_amount",

            "sales.reference_no",
            "sales.rates",
            "sales.sub_total_price",
        ...
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();*/
        return view('report.purchase_report', compact('datas','lims_warehouse_list','start_date','end_date','warehouse_id','lims_product_all'));
    }
    ...
    public function warehouseReport(Request $request)
    {
       ...
       /* $lims_sale_data = Product_Sale::where('warehouse_id', $warehouse_id)
            ->whereDate('created_at', '>=', $start_date)
            ->whereDate('created_at', '<=', $end_date)
        ...
        $lims_sale_data =
            DB::table('product_sales')
                ->where('product_sales.warehouse_id', $warehouse_id)
                ->leftJoin("sales", "product_sales.sale_id", "=", "sales.id")
                //->select('product_sales.created_at','sales.reference_no','sales.paid_amount','product_sales.qty','product_sales.net_unit_price','product_sales.tax','product_sales.total')
        ...
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        return view('report.warehouse_report', compact('warehouse_id', 'warehouse_name', 'start_date', 'end_date', 'lims_purchase_data', 'lims_product_purchase_data', 'lims_sale_data', 'lims_product_sale_data', 'lims_warehouse_list', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_return_data', 'lims_product_return_data', 'lims_expense_data'));
    }
