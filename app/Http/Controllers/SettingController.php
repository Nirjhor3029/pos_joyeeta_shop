...
class SettingController extends Controller
{
    ...
    public function extraCharge()
    {
        ...
        // return $lims_general_setting_data;
        return view('setting.extra_charge_setting', compact('lims_general_setting_data', 'lims_account_list', 'zones_array'));
    }
    public function extraChargeStore(Request $request)
    {
        if (!env('USER_VERIFIED'))
            return redirect()->back()->with('not_permitted', 'This feature is disable for demo!');
        ...
        $general_setting = GeneralSetting::latest()->first();
        $general_setting->id = 1;
        $general_setting->service_charge = $request->service_charge;
        $general_setting->vat = $request->vat;
        $general_setting->save();
