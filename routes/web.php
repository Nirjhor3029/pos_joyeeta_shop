...
// use Illuminate\Routing\Route; //never use
...
Route::get('report_quntity_update', 'SaleController@quntity_update');

Route::get('/price-update',function(){
	$sales = Sale::latest()->first();
	$rates = json_decode($sales->rates,true);
	print_r($rates);
	echo $rates['service_charge'];
	// return $sales->rates;
});
...
Route::group(['middleware' => ['auth', 'active']], function () {
	...
	Route::post('report/all_sell', [ReportController::class,'purchaseReport'])->name('report.purchase');
	...
	// edit:date- 26 feb 2022
	Route::get('setting/extra-charge-setting', [SettingController::class, 'extraCharge'])->name('setting.extraCharge');
	Route::post('setting/general_extra_charge_store', [SettingController::class, 'extraChargeStore'])->name('setting.extraChargeStore');
	...
	/*salman*/
	Route::post('disbursement/set-disburse', 'TransferController@set')->name('disbursement.set');
	Route::post('disbursement/batch-disburse', 'TransferController@makeBatch')->name('disbursement.batch');
	Route::get('disbursement/batch-list', 'TransferController@batchList')->name('disbursement.batchList');
	Route::get('disbursement/batch-list/{id}', 'TransferController@show')->name('disbursement.show');


	Route::post('disbursements/batch_payment', 'TransferController@batchPayment')->name('disbursement.payment');
	Route::get('disbursements/batch_delete/{id}', 'TransferController@batchDelete')->name('batch.delete');
	Route::post('disbursements/confirm', 'TransferController@confirmDisburse')->name('disbursement.confirmation');
	Route::get('disbursements/history', 'TransferController@disburseHistory')->name('disbursement.history');
	/*salman*/
