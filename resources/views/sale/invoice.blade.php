    ...
    <div id="receipt-data">
        <div class="centered">
            @if($general_setting->site_logo)
                <img src="{{url('public/logo/logo.png')}}" height="60" width="60" style="margin:10px 0">
            @endif
            
            <h2>{{$lims_biller_data->company_name}}</h2>
            
            <p>{{trans('file.Address')}}: Rapa Plaza, 1 Rd No. 27, Dhaka 1207
                <br>{{trans('file.Phone Number')}}: {{$lims_warehouse_data->phone}}
            </p>
        </div>
        <div>
            <p>
                Govt. of the people's republic of Bangladesh, National Board of Revenue
            </p>
            <table>
                <tbody>
                    <tr style="border: none">
                        <td style="text-align:left" colspan="2">Tax Invoice</td>
                        <td style="text-align:right">(MUSK-6.3)</td>
                    </tr>
                    <tr style="border: none">
                        <td colspan="3">
                            [Clauses(c)&(f) of Sub-rule(1) of Rule(40)]
                        </td>
                    </tr>
                    <tr style="border: none">
        ...
        <p>{{trans('file.Date')}}: {{\Carbon\Carbon::parse($lims_sale_data->created_at)->format('d-M-Y  g:i:s A') }}<br>
            {{trans('file.reference')}}: {{$lims_sale_data->reference_no}}<br>
            {{trans('file.customer')}}: {{$lims_customer_data->name}}<br>
            {{trans('Card Number')}}: {{$card_number}}<br>
            Payment Type : {{$lims_sale_data->payment_method}}<br>
            {{trans('Biller')}}: {{$lims_biller_data->name}}
        </p>
        <table>
            <tbody>
                @foreach($lims_product_sale_data as $product_sale_data)
                @php 
                $shopExplode=explode(' ', $product_sale_data->warehouse->name);
                ...
                @endphp
                <tr><td >{{$product_name}}<br>{{$product_sale_data->qty}} x {{number_format((float)($product_sale_data->total / $product_sale_data->qty), 2, '.', '')}}</td>
                    <td style="vertical-align:bottom">Shop<br>{{$shop_name}} </td>
            ...
            <tfoot>
                <tr>
                    ...
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->sub_total_price, 2, '.', '')}}</th>
                </tr>
                ...
                <tr>
                    <td></td>
                    <td style="text-align:right">+ {{trans('file.VAT')}} ({{ $general_setting->vat }}%)</td>
                    <td style="text-align:right">{{number_format((float)$lims_sale_data->total_vat, 2, '.', '')}}</td>
                </tr>
                @if($lims_sale_data->order_tax)
                <tr>
                    <th colspan="2">{{trans('file.Card charge')}}</th>
            ...
            <tbody>
                ...
                {{-- <tr>
                    <td class="centered" colspan="3">
                        No money refund is possible. Exchange possible within 7days of sell. Please bring original money receipt and the package.
                    </td>
                </tr> --}}
                <tr>
                    <td class="centered" colspan="3">
                        VAT Note: {{$general_setting->vat}}% retailer VAT applied on all items.
                    </td>
                </tr>
                @endforeach
            </tbody>
