    ...
    <div class="table-responsive mb-4">
        <table id="report-table" class="table table-hover">
            ...
            <tbody>
                ...
                <?php
                // calculate 
                $product_price = $data->price * $data->total_quantity;

                $vat_rate = 0;  
                $vat_per_product = 0;
                $discount_per_product=0;
                if(!is_null($data->rates)){
                    $rates = json_decode($data->rates);
                    $vat_rate = $rates->vat_rate;
                    $vat_per_product = $product_price * ($vat_rate/100);
                    
                    if(is_null($data->order_discount)){
                        $discount_per_product=0;
                    }
                    else {
                        //'sub_total_price' is the total without VAT & Discount
                        if($data->sub_total_price){ // sub total : 0 not allowed 
                            $discount_per_product = ($product_price * $data->order_discount)/$data->sub_total_price;
                            $discount_per_product = round($discount_per_product);
                        }
                    }
                }
                ...
                <tr>
                    ...
                    <td>{{$data->cost * $data->total_quantity}}</td>
                    <td>{{$product_price}}</td>
                    
                    <td>{{$vat_per_product}}</td>
                    <td>{{ $product_price +  $vat_per_product}}</td>
                    <td>{{$discount_per_product}}</td>
                    {{-- <td>{{$data->paid_amount}}</td> --}}
                    <td>{{($product_price +  $vat_per_product) - $discount_per_product}}</td>
...
<script type="text/javascript">
    ...
    function datatable_sum(dt_selector, is_calling_first) {
        if (dt_selector.rows( '.selected' ).any() && is_calling_first) {
            ...
            $( dt_selector.column( 9 ).footer() ).html(dt_selector.column( 9, {page:'current'} ).data().sum());
